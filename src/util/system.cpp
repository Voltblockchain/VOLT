...
const char * const VOLT_CONF_FILENAME = "volt.conf";
const char * const VOLT_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "voltd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\VOLT
// Windows >= Vista: C:\Users\Username\AppData\Roaming\VOLT
// Mac: ~/Library/Application Support/VOLT
// Unix: ~/.volt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VOLT";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VOLTParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VOLTParams
    // Mac: ~/Library/Application Support/VOLTParams
    // Unix: ~/.volt-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VOLTParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "VOLTParams";
#else
    // Unix
    return pathRet / ".volt-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/volt/sapling-spend.params";
        sapling_output = "/usr/share/volt/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/volt/sapling-spend.params";
            sapling_output = "/usr/local/share/volt/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", VOLT_MASTERNODE_CONF_FILENAME));
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "server=1\n"
                                    ...
                                    "port=7160\n\n"
                                    "maxconnections=500\n\n"
                                    "addnode=139.59.142.109\n"
                                    "addnode=46.101.108.156\n"
                                    "addnode=104.248.29.44\n"
                                    "addnode=104.248.21.205\n"
                                    "addnode=46.101.98.149\n"
                                    "addnode=170.64.232.197\n";
