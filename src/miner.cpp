...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("VOLTMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("VOLTMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("VOLTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("volt-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running VOLTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("VOLTMiner exception");
    } catch (...) {
        LogPrintf("VOLTMiner exception");
    }

    LogPrintf("VOLTMiner exiting\n");
}
