 ...
 */
const char * const VOLT_PID_FILENAME = "volt.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", VOLT_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("volt-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", VOLT_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", VOLT_PID_FILENAME));
#endif
...
{
    util::ThreadRename("volt-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that VOLT is running in a usable environment with all
...
{
    // Make sure only a single VOLT process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", VOLT_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if VOLT is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if VOLT is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // VOLT: load previous sessions sporks if we have them.
