...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " volt-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  volt-tx [options] <hex-tx> [commands]  Update hex-encoded volt transaction\n" +
                               "or:     volt-tx [options] -create [commands]   Create hex-encoded volt transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded volt transaction
