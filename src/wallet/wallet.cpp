...
/**
 * Fees smaller than this (in uvlt) are considered zero fee (for transaction creation)
                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.GetMNCollateral(chainHeight)) {
        strError = strprintf("Invalid collateral tx value, must be %s VLT", FormatMoney(Params().GetConsensus().GetMNCollateral(chainHeight)));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-volt-address
...
bool CWallet::CreateCoinstakeOuts(const CVltStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CVltStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_VOLT && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_VOLT);
        }
