...
#include "qt/volt/dashboardwidget.h"
#include "qt/volt/forms/ui_dashboardwidget.h"
#include "qt/volt/sendconfirmdialog.h"
#include "qt/volt/txrow.h"
...
DashboardWidget::DashboardWidget(VOLTGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareVlt, "square-chart-vlt");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelVlt, "text-chart-vlt");
    ...
    setCssProperty(ui->labelAmountVlt, "text-stake-vlt-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &VOLTGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(12,23,26);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isVlt = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VLT, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            vlt = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalVlt += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesVlt.append(vlt);
        chartData->valuesMN.append(mn);

        int max = std::max(vlt, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(54,158,181));
    set1->setColor(QColor(134,231,252));
    ...
    set0->append(chartData->valuesVlt);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalVlt > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountVlt, "text-stake-vlt");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountVlt, "text-stake-vlt-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountVlt, ui->labelAmountMN});
    ui->labelAmountVlt->setText(GUIUtil::formatBalance(chartData->totalVlt, nDisplayUnit));
