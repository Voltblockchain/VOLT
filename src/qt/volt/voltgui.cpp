...
#include "qt/volt/defaultdialog.h"
#include "shutdown.h"
...
const QString VOLTGUI::DEFAULT_WALLET = "~Default";

VOLTGUI::VOLTGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void VOLTGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &VOLTGUI::toggleHidden);
...
void VOLTGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &VOLTGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &VOLTGUI::changeTheme);
    //connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &VOLTGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &VOLTGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &VOLTGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &VOLTGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &VOLTGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &VOLTGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &VOLTGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &VOLTGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &VOLTGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &VOLTGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &VOLTGUI::execDialog);
}


void VOLTGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void VOLTGUI::handleRestart(QStringList args)
{
...
void VOLTGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &VOLTGUI::message);
...
void VOLTGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &VOLTGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &VOLTGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void VOLTGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void VOLTGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void VOLTGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &VOLTGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &VOLTGUI::show);
                e->ignore();
            }
        }
    }
...
void VOLTGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void VOLTGUI::messageInfo(const QString& text)
{
...
void VOLTGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool VOLTGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void VOLTGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void VOLTGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VOLTGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void VOLTGUI::goToSend()
{
    showTop(sendWidget);
}

void VOLTGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void VOLTGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void VOLTGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void VOLTGUI::goToGovernance()
{
    showTop(governancewidget);
}

void VOLTGUI::goToSettings(){
    showTop(settingsWidget);
}

void VOLTGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void VOLTGUI::goToReceive()
{
    showTop(receiveWidget);
}

void VOLTGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void VOLTGUI::showTop(QWidget* view)
{
...
void VOLTGUI::changeTheme(bool isLightTheme)
{
...
void VOLTGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool VOLTGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void VOLTGUI::showHide(bool show)
{
...
int VOLTGUI::getNavWidth()
{
    return this->navMenu->width();
}

void VOLTGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void VOLTGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void VOLTGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool VOLTGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &VOLTGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &VOLTGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &VOLTGUI::message);
    connect(topBar, &TopBar::message, this, &VOLTGUI::message);
    connect(sendWidget, &SendWidget::message,this, &VOLTGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &VOLTGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &VOLTGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &VOLTGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &VOLTGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &VOLTGUI::incomingTransaction);

    return true;
}

bool VOLTGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void VOLTGUI::removeAllWallets()
{
    // Single wallet supported.
}

void VOLTGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(VOLTGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void VOLTGUI::subscribeToCoreSignals()
{
...
void VOLTGUI::unsubscribeFromCoreSignals()
{
