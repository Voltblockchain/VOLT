...
#include "qt/volt/forms/ui_addresseswidget.h"
#include "qt/volt/addressfilterproxymodel.h"
#include "qt/volt/addresslabelrow.h"
#include "qt/volt/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(VOLTGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto vltAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(vltAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(vltAdd);
        if (walletModel->updateAddressBookLabels(vltAdd, label.toUtf8().constData(),
