...
#include "qt/volt/send.h"
#include "qt/volt/forms/ui_send.h"
#include "qt/volt/addnewcontactdialog.h"
#include "qt/volt/qtutils.h"
#include "qt/volt/sendchangeaddressdialog.h"
#include "qt/volt/optionbutton.h"
#include "qt/volt/sendconfirmdialog.h"
#include "qt/volt/guitransactionsutils.h"
...
SendWidget::SendWidget(VOLTGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a VOLT URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-vlt");
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent VLTs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield VLTs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent VLTs to shield."));
    }
}
...
void SendWidget::onVLTSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto vltAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(vltAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(vltAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vltAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
