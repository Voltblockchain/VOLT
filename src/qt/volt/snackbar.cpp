...
#include "qt/volt/forms/ui_snackbar.h"
#include "qt/volt/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(VOLTGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &VOLTGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
