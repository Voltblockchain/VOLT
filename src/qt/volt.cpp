...
#include "qt/volt/splash.h"
#include "qt/volt/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/volt/governancemodel.h"
...
{
    return QCoreApplication::translate("volt-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in volt.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in volt.qrc)
...
/** Class encapsulating VOLT Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (VOLTGUI)
...
{
    window = new VOLTGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &VOLTGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &VOLTGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(VOLTGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(VOLTGUI::DEFAULT_WALLET);
        ...
        // VOLT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VOLTGUI::handlePaymentRequest);
        connect(window, &VOLTGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. VOLT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(volt_locale);
    Q_INIT_RESOURCE(volt);
    ...
    /// 6. Determine availability of data directory and parse volt.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", VOLT_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // volt: links repeatedly have their payment requests routed to this process:
