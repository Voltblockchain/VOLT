...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::VLT);
    ...
    if (settings.contains("nAnonymizeVOLTAmount") || reset)
        gArgs.SoftSetArg("-anonymizevoltamount", settings.value("nAnonymizeVOLTAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in volt.cpp during shotdown to also remove the window positions
