...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select VLT Outputs to Spend" :
            "Select Shielded VLT to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::VLT;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show VOLT address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
