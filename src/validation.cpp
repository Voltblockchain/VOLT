...
#if defined(NDEBUG)
#error "VOLT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvlt) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k VLT
    ...
    if (nHeight > 0 && nHeight < nLast) {
        return 720 * COIN;
    } else if (nHeight >= nLast && nHeight <= 55000) {
        return 2 * COIN;
    } else if (nHeight > 55000 && nHeight <= 120000) {
        return 2.5 * COIN;
    } else if (nHeight > 120000 && nHeight <= 200000) {
        return 3.25 * COIN;
...
{
    util::ThreadRename("volt-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // VOLT Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nVOLTBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nVOLTBadBlockBits)) {
            // accept VOLT block minted with incorrect proof of work threshold
            return true;
        }
