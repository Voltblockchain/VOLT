...
*Setup instructions for a gitian build of VOLT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the VOLT
...
to the VOLT GitHub release page.
...
- [Building VOLT Core](#building-volt-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/volt-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/volt-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/volt-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/volt-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/volt-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/volt-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/volt-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@volt.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/volt-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/volt-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/volt-osx-*-build.assert
```
