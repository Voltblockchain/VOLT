...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/VOLTParams"
    else
        PARAMS_DIR="$HOME/.volt-params"
    fi
fi
...
Failed to fetch the VOLT zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
VOLT - fetch-params.sh

This script will fetch the VOLT zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common VOLT zkSNARK parameters. Note that it is
