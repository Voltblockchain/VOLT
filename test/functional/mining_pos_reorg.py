...
class ReorgStakeTest(VOLTTestFramework):
    ...
    def check_money_supply(self, expected_vlt):
        # verify that nodes have the expected VLT supply
        vlt_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(vlt_supply, [DecimalAmt(expected_vlt)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that VLT supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check VLT supply...")
